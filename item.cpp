...
item::item() : ID(""), itemName(""), category(""), description(""), highestBidder(""), startingBid(0), currentBid(0), minBuyerRating(0), endTime(0), startTime(0) {
    time(&current_time);
}

item::item(string ID, string itemName, string category, string description, string highestBidder, int startingBid, int currentBid, int minBuyerRating, time_t endTime, time_t startTime)
    : ID(ID), itemName(itemName), category(category), description(description), highestBidder(highestBidder), startingBid(startingBid), currentBid(currentBid), minBuyerRating(minBuyerRating), endTime(endTime), startTime(startTime) {
    time(&current_time);
}
