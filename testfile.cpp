...
public:
    User(string uname = "", string pwd = "", string fname = "", int phone = 0, string mail = "", int rate = 0, int credits = 0, int won = 0)
        : username(uname), password(pwd), fullName(fname), phoneNumber(phone), email(mail), rating(rate), creditPoints(credits), itemsWon(won) {}
...
User* loadUser(const string& username, const string& password) {
    ...
    while (getline(inFile, line)) {
        ...
        if (uname == username && pwd == password) {
            string fullName, email, idNumber;
            int phoneNumber, rating, creditPoints;
            ...
            inFile.close();
            return new User(uname, pwd, creditPoints, 0); // Adjust for your attributes
        }
    }
    inFile.close();
